...
def main():
    while True:
        ...
        # Print Choices
        result_player = cur.execute("select name from hero").fetchone()
        if result_player[0] != 'Player': 
            console.print("([red]1[/red]) Play Game ([green]" + result_player[0] + "[/green])")
        ...
        if ans == '2':
            createhero()
        elif ans == '1' and result_player[0] != 'Player':
            adventuremenu()
        elif ans == '3':
            gameoptions()
        elif ans == '4':
